...
export abstract class ResourceService<SPEC, STATE> {
  ...
  abstract destroy(...stateParts: StatePart<STATE>[]): Promise<void>;

  /**
   * @return 可能返回多个实际的同名云资源，因为一个资源可能被非正常的多次创建，重复问题留给上层程序判断解决
   */
  abstract refresh(specPart: SpecPart<SPEC>): Promise<StatePart<STATE>[]> ;
}
...
export abstract class Provider {
   ...
   */
  abstract listProvisionedResources(): Promise<StatePart<unknown>[]>;
}
