...
import {ResourceConfig, Resource, ProviderConfig} from "@qpa/core";
...
export class CvmFactory extends _TencentCloudResourceFactory {
  ...
  constructor(tc: _TencentCloud,
              private readonly provider: ProviderRuntime<ProviderConfig>, private cvmClient: _CvmClientWrap) {
    super(tc);
  }
  ...
  async instance(expected: ResourceConfig<CvmInstanceSpec>): Promise<Resource<CvmInstanceSpec, CvmInstanceState>> {
    const service = this.provider.resourceServices.get(TencentCloudResourceType.cvm_instance) as _CvmInstanceService;
