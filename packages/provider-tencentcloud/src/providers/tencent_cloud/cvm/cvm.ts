...
import {PlannedResource, SpecPart, StatusPart} from "@qpa/core";
...
export class CvmInstanceService extends TaggableResourceService<Cvm,CvmState> {
    ...
    destroy(resource: PlannedResource<Cvm, CvmState>): Promise<void> {
        console.log(resource)

        throw new Error("not implements")
    }

    refresh(resource: PlannedResource<Cvm, CvmState>): Promise<void> {
        console.log(resource)
