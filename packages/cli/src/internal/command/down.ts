...
export default function registerCommand<Vars>(parentCommand: Command,cli: Cli): void {
  // 在父命令上创建 'plan' 子命令
  parentCommand.command('down')
  .description('下线(销毁)所有资源')
  ...
  .action(async () => {
    ...
    for (const resourceInstance of cli.project.resourceInstances) {
      console.log(`计划下线(销毁)： ${resourceInstance.resourceType.name} ${resourceInstance.name}`)
    }
    await cli.project.down();
    console.log(`下线(销毁)完成`)
